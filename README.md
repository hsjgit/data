...
*删除就是就是用待删除元素的后以后元素代替，之后在依次替代前卖一个元素，最后数组大小减一*
...
链表就是多个节点组成的线性表，每个节点都有连个值，一个是节点本身的值，和当前节点的下一个节点引用，在Java中对象就相当于指向某个堆内存的状态就相当于指针指向这块堆内存。

在一个类中应该含有两个成员变量，一个就是当前节点的值，另一个成员变量则指向下一个节点。

```java
/**
* 这里使用一个内部类作为一个节点，是为了保证每个节点只有本类才可以访问
* 也可以使用一个外部的公有类
*/
private class Node{
        private E e;
...
添加节点有两个办法，一个就是在当前节点的下一个节点的位置添加上，只需要让上一个节点中成员变量为指向下一个节点的的对象。